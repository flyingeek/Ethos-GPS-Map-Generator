<script>
  ...
  bottomRight = { lat = ${bottomRight.lat}, lon = ${bottomRight.lon} }
    ...
    async function prepareMapFrameCapture() {
        if (!map) return;
    ...
    function drawNorthArrow(ctx, width, height, bearing) {
        const size = Math.min(
            64,
    ...
    async function createBmpBlob(depth = 16) {
        if (!mapViewport) {
        ...
        let canvas;
        try {
            // Capture directly from the WebGL map canvas to avoid html2canvas cloning issues.
        ...
        const padding = (4 - ((mapWidth * bytesPerPixel) % 4)) % 4;
        const fileSize =
            headerSize + (mapWidth * bytesPerPixel + padding) * mapHeight;
    ...
    async function handleDownloadZip() {
        const baseName = cleanBaseName();
        const bmpBlob = await createBmpBlob(16);
